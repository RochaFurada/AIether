    ...
    def initialize_new_layer_from_history(self, new_layer: nn.Module, param_history: dict, strategy: str):
        ...
        with torch.no_grad():
            for name, param in new_layer.named_parameters():
                ...
                # Stack temporal snapshots with safety
                try:
                    logger.info(f"      📊 Stacking history for {name}: {len(history_list)} snapshots")
                    W_hist = np.stack(history_list, axis=0)
                    logger.info(f"      ✅ Stacked shape: {W_hist.shape}")
                except Exception as e:
                    logger.error(f"      ❌ Failed to stack {name}: {e}")
                    param.data.copy_(torch.from_numpy(history_list[-1]).to(param.device))
                    continue
                
                # Apply geometric extrapolation with protection
                try:
                    logger.info(f"      🔮 Extrapolating {name}...")
                    W_new_np, metrics = self.geometric_extrapolator.extrapolate(W_hist)
                    logger.info(f"      ✅ Extrapolation completed")
                except Exception as e:
                    logger.error(f"      ❌ Extrapolation failed for {name}: {e}")
                    # Fallback: use last snapshot
                    W_new_np = history_list[-1]
                    metrics = {'tau': 0, 'kappa_eff': 0, 'delta_norm': 0}
